...
class HWConsumablesPanel(ConsumablesPanelMeta):
    ...
    def _populate(self):
        ...
        if hwBattleGuiCtrl:
            hwBattleGuiCtrl.onUpdateVehicleModifiers += self._onUpdateVehicleModifiers
    ...
    def _updateHWEquipmentKeyHandlers(self, updateSlots=None):
        hwEquipmentRange = range(self._HW_EQUIPMENT_START_IDX, self._HW_EQUIPMENT_END_IDX + 1)
    ...
    def _updateEquipmentSlot(self, idx, item, forceUpdate=False):
        ...
        if not forceUpdate and currStage == prevStage:
            return
        count = item.getUsageCost() if self._hasUsageCost(item) else 0
    ...
    def _onCurrentShellChanged(self, intCD):
        super(HWConsumablesPanel, self)._onCurrentShellChanged(intCD)
        for shellIntCD, count in self._shellsCount.items():
            self._updateShellGlow(shellIntCD, count, count)
    ...
    def _onUpdateHWEquipmentTimer(self):
        return self._onUpdateHWEquipment()

    def _onUpdateHWEquipment(self, onModifier=False):
        ctrl = self.sessionProvider.shared.equipments
        if ctrl is not None:
            for intCD, item in ctrl.iterEquipmentsByTag('hwEquipment'):
                if intCD not in self._cds:
                    continue
                idx = self._cds.index(intCD)
                if onModifier or self._hwEquipmentGlowAllowed.get(idx, None) != self._isGlowAllowed(item):
                    self._updateEquipmentSlot(idx, item, True)
                if onModifier:
                    self.as_updateTooltipS(idx, self._buildEquipmentSlotTooltipText(item))
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        if self._hasDuration(item):
            duration = int(item.getDurationSeconds())
            if duration > 0:
                body = ('\n').join((
                 body,
                 backport.text(R.strings.halloween_battle.consumablesPanel.tooltip.durationTime(), time=str(duration))))
        cooldownSeconds = int(item.getCooldownSeconds())
        if cooldownSeconds:
            tooltipStr = R.strings.halloween_tooltips.consumables.params.cooldownSeconds()
            paramsString = backport.text(tooltipStr, cooldownSeconds=str(cooldownSeconds))
            body = ('\n').join((body, paramsString))
        if self._hasUsageCost(item):
            usageCost = int(item.getUsageCost())
            if usageCost:
    ...
    def _onUpdateVehicleModifiers(self, _):
        self._onUpdateHWEquipment(True)