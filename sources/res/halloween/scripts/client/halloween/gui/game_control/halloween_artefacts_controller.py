...
BONUS_ORDER = [
 'lootBox', 'dossier', HWStoryChoiceSettings.FAKE_MEDAL, 'vehicles', 'slots', 'cerfToken', 'tmanToken', 'tankmen', 'crewSkins', ArtefactsSettings.CREW_100, 'berths', 'customizations', 'crewBooks', PREMIUM_ENTITLEMENTS.VIP, PREMIUM_ENTITLEMENTS.PLUS, PREMIUM_ENTITLEMENTS.BASIC, Currency.BPCOIN, 'battlePassPoints', ArtefactsSettings.KEY_TOKEN, Currency.CRYSTAL, Currency.GOLD, Currency.CREDITS, 'xp', Currency.FREE_XP, Currency.EQUIP_COIN, 'battle_bonus_x5', 'crew_bonus_x3', 'battlePassQuestChainToken', 'tokens', 'battleToken', 'vehicleXP', 'tankmenXP', 'goodies', 'items', 'blueprints_universal'] + _blueprints_national_order + ['blueprints', 'blueprintsAny']
...
def getTokenValue(bonus):
    token = first(bonus.getTokens().keys(), '')
...
class HalloweenArtefactsController(IHalloweenArtefactsController, IGlobalListener):
    ...
    def getOpenedArtefactToken(self, artefactID):
        return artefactID + self._getConfig().get('openedSuffix', '')
