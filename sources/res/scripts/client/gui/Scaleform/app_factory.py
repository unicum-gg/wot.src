...
from gui.Scaleform.daapi.settings.views import BATTLE_SHARED_LAYOUTS_CONFIG, VIEW_ALIAS, RANDOM_BATTLE_PAGE_CONFIG, VIEW_BATTLE_PAGE_CONFIG_BY_ARENA_GUI_TYPE
...
if typing.TYPE_CHECKING:
    from gui.impl.pub import MainWindow
...
class AS3_AppFactory(IAppFactory):
    ...
    def __closeApp(self, appNS, app):
        _logger.info('Destroying app: %s', appNS)
        app.close()
        self.__gui.layoutManager.destroyLayouts(appNS)

    def destroyLobby(self):
        if _SPACE.SF_LOBBY in self.__apps:
            lobby = self.__apps[_SPACE.SF_LOBBY]
            if lobby:
                self.__closeApp(_SPACE.SF_LOBBY, lobby)
    ...
    def destroyBattle(self):
        if _SPACE.SF_BATTLE in self.__apps:
            battle = self.__apps[_SPACE.SF_BATTLE]
            if battle:
                self.__closeApp(_SPACE.SF_BATTLE, battle)
    ...
    def getMainWindow(self):
        windows = self.__gui.windowsManager.findWindows(lambda w: w.typeFlag == WindowFlags.MAIN_WINDOW)
        if not windows:
    ...
    def _loadBattlePage(self, arenaGuiType):
        ...
        for layoutName in BATTLE_SHARED_LAYOUTS_CONFIG:
            if isReplay and self.__gui.layoutManager.getLayoutByName(_SPACE.SF_BATTLE, layoutName):
                continue
            self.__gui.layoutManager.createAbsoluteLayout(_SPACE.SF_BATTLE, layoutName)

        mainWindow = self.getMainWindow()
        for viewAlias in VIEW_BATTLE_PAGE_CONFIG_BY_ARENA_GUI_TYPE.get(arenaGuiType, RANDOM_BATTLE_PAGE_CONFIG):
            event = events.LoadViewEvent(SFViewLoadParams(viewAlias, parent=mainWindow))
