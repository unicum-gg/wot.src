...
class VIEW_ALIAS(COMMON_VIEW_ALIAS):
    ...
    RANDOM_BATTLE_HUD = 'randomBattleHud'
...
def addViewBattlePageConfigByArenaGUIType(arenaGuiType, config, personality):
    if arenaGuiType in VIEW_BATTLE_PAGE_CONFIG_BY_ARENA_GUI_TYPE:
        raise SoftException(('VIEW_BATTLE_PAGE_CONFIG_BY_ARENA_GUI_TYPE already has arenaGuiType:{guiType}. Personality: {p}').format(guiType=arenaGuiType, p=personality))
    ...
    msg = ('arenaGuiType:{arenaGuiType} was added to VIEW_BATTLE_PAGE_CONFIG_BY_ARENA_GUI_TYPE. Personality: {p}').format(arenaGuiType=arenaGuiType, p=personality)
    logging.debug(msg)