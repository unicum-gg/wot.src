    ...
    def _validateTooltipsData(self, tooltipsSettings):
        for itemName, data in viewitems(tooltipsSettings):
            if not i18n.doesTextExist(data.name):
                _logger.error("[ERROR] BattleAbilityTooltipManager: %s: Localization for '%s' not found.", itemName, data.name)
    ...
    def createBattleAbilityTooltipRenderers(self, skillInfo, staticBlock, dynamicBlock, level=1):
        ...
        for tooltipIdentifier in curLvlEq.tooltipIdentifiers:
            ...
            if tooltipInfo is None:
                _logger.error('[ERROR] createBattleAbilityTooltipRenderers: Failed to find tooltipInfo %(ttid)s for %(us)s (%(name)s).', {'ttid': tooltipIdentifier, 'us': curLvlEq.userString, 
                   'name': curLvlEq.name})
                continue
