...
_ReceiverInBattle = namedtuple('_ReceiverInBattle', ('name', 'label', 'modifiers',
                                                     'bwModifiers', 'order'))
...
def _readReceiverValue(xmlCtx, section, settings=None):
    ...
    return (
     name,
     _ReceiverInBattle(name, label, modifiers, bwModifiers, valueSec.readInt('order')))
...
_BATTLE_ITEM_READERS = {'messageFormat': _xml_helpers.readUnicodeItem, 
   ...
   'chatIsLockedParentalControlToolTipText': _xml_helpers.readI18nStringItem, 
   'chatIsLockedCountryControlToolTipText': _xml_helpers.readI18nStringItem, 
   'chatIsLockedOtherControlToolTipText': _xml_helpers.readI18nStringItem, 
